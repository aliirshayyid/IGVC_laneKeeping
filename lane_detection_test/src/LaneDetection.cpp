  ...
  kd_tree_(new pcl::search::KdTree<pcl::PointXYZ>)
{
  ...
  pn.param("camera_name", camera_name_, std::string("camera"));
...
void LaneDetection::segmentImage(const cv::Mat& raw_img, cv::Mat& bin_img)
{
  ...
  //This portion deals only with detecting the HSV for yellow lanes, not needed for the competion and causes noise
  ...
  mask(cv::Rect(0, cfg_.mask_bot_height, mask.cols, mask.rows - cfg_.mask_bot_height - 1)) = 0;

  mask(cv::Rect(0, 0, mask.cols, mask.rows - cfg_.mask_top_height - 1)) = 0;

  //Replace 0 with 'yellow_bin_img to include yellow lanes
  cv::bitwise_or(white_bin_img, 0, bin_img, mask);
