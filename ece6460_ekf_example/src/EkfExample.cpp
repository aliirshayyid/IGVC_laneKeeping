...
EkfExample::EkfExample(ros::NodeHandle n, ros::NodeHandle pn)
{
  ...
  pub_fix_ = n.advertise<gps_common::GPSFix>("enhanced_fix", 1);
...
void EkfExample::recvFix(const sensor_msgs::NavSatFixConstPtr& msg)
{
  ...
  //I believe this is only us to compare ekd results with raw results, plan to elminate
...
void EkfExample::timerCallback(const ros::TimerEvent& event)
{
  ...
  //Publish GPS Common message
  gps_common::GPSFix enhanced_fix;
  double new_lat;
  double new_lon;
  tf2::Vector3 filtered_coords = tf2::Vector3(X_(0), X_(1), 0) + ref_utm_vect_;
  gps_common::UTMtoLL(filtered_coords.getY(), filtered_coords.getX(), utm_zone, new_lat, new_lon);
  enhanced_fix.header = ekf_transform.header;
  enhanced_fix.track = X_(2);
  enhanced_fix.latitude = new_lat;
  enhanced_fix.longitude = new_lon;
  enhanced_fix.speed = X_(3);
  pub_fix_.publish(enhanced_fix);
}
